...
(define (desugar-dots (obj : ExprP) (field : symbol) (args : (listof ExprP))) : ExprC
	(local ([define ids (make-ids (length args))]
          [define id-exps (map IdC ids)])
    (cascade-lets ids (map desugar args)
			(GetFieldC (desugar obj) (IdC field))
		) 
	)
)
    ...
    [StrP (s) (StrC s)]
				...
				[BracketLHS (obj field) 
					(LetC 'tmpfield (desugar field)
						(IfC (check-type (IdC 'tmpfield) "string")
							...
							(ErrorC (StrC (string-append "warning " (to-string (desugar field)))))
						)		
					)		
				]
				[DotLHS (obj field) (SetFieldC (desugar obj) (IdC field) (desugar value))]
		...
		[PrimAssignP (op lhs value)
			(type-case LHS lhs
				...
				[DotLHS (obj field) (desugar-prim-assign-dot (desugar obj) (IdC field) op (desugar value))]
				[IdLHS (id) (desugar-prim-assign-id id op (desugar value))]
			)
		]

 	  [PostIncP (lhs) (desugar-prim-assign-id lhs '+ (NumC 1))] ;;doesnt work
		...
		[DotMethodP (obj field args)  (desugar-dots obj field args)]
		[BrackMethodP (obj field args) (StrC "brackmethodP")]
	  ...
	  ;;[SetFieldC (obj : ExprC) (field : ExprC) (value : ExprC)]
		...
		[ObjectP (fields) (ObjectC (map (lambda (field) (desugar-field field)) fields))]
